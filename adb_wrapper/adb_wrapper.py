...
class AdbShell(object):
    ...
    def _read(self, q):
        ...
        while 1:
            try:
                tmp = q.get_nowait()
                self.logger.debug("out: {!r}".format(tmp))
            except Empty:
                break
            else:
                out.append(tmp)
    ...
    def kill(self):
        ...
        if self.stdout_t.isAlive():
            ...
            try:
                self.stdout_t.join(10)
            except RuntimeError as err:
                self.logger.warning("AdbShell Kill stdout Timeout: %s", err)
        if self.stderr_t.isAlive():
            self.stderr_stop.set()
            try:
                self.stderr_t.join(10)
            except RuntimeError as err:
                self.logger.warning("AdbShell Kill stderr Timeout: %s", err)
...
class AdbWrapper(BaseWrapper):
    ...
    def connect(self, device=None):
        ...
        '''
        if u'.' not in device:
            self.logger.warning("Non IP device, treat as USB device, skip connect")
            return device
        ...
        elif u'No route to host' in stdout:
            self.logger.error("No route to host")
            raise AdbConnectFail("No route to host", stdout, stderr)
        else:
    ...
    def install(self, apkfile, forward=False, replace=False, test=False,
        ...
        if u'No APK file on command line' in stderr:
            ...
            self.logger.error("install: %s", reason)
            raise AdbFailException(reason, stdout, stderr)
        elif u'Failed to parse' in stderr:
            reason = stderr[stderr.find(u'Failed to parse'):]
            self.logger.error("install: %s", reason)
            raise AdbFailException(reason, stdout, stderr)
    ...
    def enable_verity(self, device=None):
        ...
        elif u'Successfully enabled verity' in stdout or stdout == '':
            self.logger.info("enable-verity: success")
        elif u'verity is already enabled' in stdout or stdout == '':
