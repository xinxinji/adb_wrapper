...
class FastbootWrapper(BaseWrapper):
    ...
    def _run_fb_cmd(self, cmdlist, timeout, ignore_waitfor=False):
        '''
        ...
        '''
        try:
            stdout, stderr = self._command_blocking(cmdlist=cmdlist, timeout=timeout, ignore_waitfor=ignore_waitfor)
    ...
    def getvar(self, variable, device=None, timeout=None, *args, **kwargs):
        ...
        if variable == u'all':
            va_list = re.findall(r'\(bootloader\) (\w)+: (.+)', stderr.lower())
    ...
    def format(self, partition, fs_type=None, size=None, device=None, timeout=None, *args, **kwargs):
        ...
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout)
        std = stdout + stderr
        if u'formatting is not supported for file system with type' in std.lower():
    ...
    def reboot(self, target=None, device=None, timeout=None, *args, **kwargs):
        ...
        _timeout = timeout if timeout else self._common_fastboot_timeout
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout, ignore_waitfor=True)
        std = stdout + stderr
    ...
    def continue_(self, device=None, timeout=None, *args, **kwargs):
        ...
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout)
        std = stdout + stderr
        if u'boot' in std.lower() and u'finished' in std.lower():
    ...
    def boot(self, kernel, ramdisk=None, second=None, device=None, timeout=None, *args, **kwargs):
        ...
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout)
        std = stdout + stderr
        if u'booting' in std.lower() and u'OKAY' in std and u'finished' in std.lower():
    ...
    def flash(self, partition, filename=None, device=None, timeout=None, *args, **kwargs):
        ...
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout)
        std = stdout + stderr
        if u'sending' in std.lower() and u'writing' in std.lower() and u'OKAY' in std:
    ...
    def flashall(self, device=None, timeout=None, reboot=True, *args, **kwargs):
        ...
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout)
        std = stdout + stderr
        if u'OKAY' in std and 'finished' in std.lower():
    ...
    def set_active(self, slot, device=None, timeout=None, *args, **kwargs):
        ...
        _timeout = timeout if timeout else self._common_fastboot_timeout
        stdout, stderr = self._run_fb_cmd(cmdlist, _timeout)
        if u'OKAY' in stderr and 'finished' in stderr.lower():
    ...
    def flashing_get_unlock_ability(self, device=None, timeout=None, *args, **kwargs):
        ...
        std = stdout + stderr
        if u'OKAY' in std and 'finished' in std.lower() and abi_re.search(stderr):
